...
if( !class_exists( 'CP_REFTRACK_BaseClass' ) )
{
    
    class CP_REFTRACK_BaseClass {       
        ...
        {
            global $wpdb;
            $results = $wpdb->get_results(  $wpdb->prepare("SHOW columns FROM `".sanitize_key($table)."` where field=%s", $field) );    
            if (!count($results))
            {               
        ...
        {   
            ...
            $value = apply_filters( 'cpreftrack_get_option', $value, $field, $this->item );    
